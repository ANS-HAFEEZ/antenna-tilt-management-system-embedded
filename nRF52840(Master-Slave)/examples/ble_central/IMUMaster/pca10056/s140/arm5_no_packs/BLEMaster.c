...
//                      );
				 ...
				 {
						//NRF_LOG_INFO("Something is here"); NRF_LOG_FLUSH();
				 } break;
...
static void ble_nus_chars_received_uart_print(uint8_t * p_data, uint16_t data_len){
	ret_code_t ret_val;
	//NRF_LOG_INFO("Receiving data %s   count ->%d\n",p_data,countconn++);
	
	uint16_t DLen = strlen(sleep);
	if(strstr((char *)p_data,"roll") && strstr((char *)p_data,"tilt")){
		for(int i=0;i<data_len;i++){
			S_ANT[i]=p_data[i];
		}
		ISBLEDataReceived=true;
		ble_nus_c_string_send(&m_ble_nus_c,(uint8_t *)sleep, DLen);
		ISScanning=false;
		NRF_LOG_INFO("Sending Sleep Command\n");
	}
}

void sendBLECMD(){
	char send[]="sendme";
	uint16_t DLen= strlen(send);
	ble_nus_c_string_send(&m_ble_nus_c,(uint8_t *)send, DLen);
	nrf_delay_ms(2000);
}
